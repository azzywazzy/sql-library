...
-- show all books in the database with title keyword "The"
select i.title, a.author_first, a.author_last, i.description, i.isbn, i.is_available 
from author a join book_author b on a.author_id = b.author_id join item i on b.isbn = i.isbn 
where title like "%the%";

-- show all copies of "Northern lights"
select i.title, c.item_barcode, l.branch_name from item i join copy c on i.isbn = c.isbn 
join location_id l on c.branch_id = l.branch_id where i.title = 'Northern lights';

-- show loan history for member
select title, author_first, author_last, loan_date, due_date, returned_date from loan l 
join copy c on l.item_barcode = c. item_barcode join item i on c.isbn = i.isbn join book_author b 
on i.isbn = b.isbn join author a on b.author_id = a.author_id where card_number = 10000003;


-- library admin assistant and above
...
-- return a book
update loan set returned_date = '2023-03-28' where item_barcode = 20000006;
